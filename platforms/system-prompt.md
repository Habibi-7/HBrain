...
<!--
  Generic system prompt addition. Works with any agent that accepts a system prompt.
  Paste this into your agent's system prompt, API system message, or equivalent.
  Change the `agent` field value in the example below to match your agent's name.
-->
...
| `source` | Yes | `agent` |
| `agent` | Yes | Your agent's name (e.g. `gpt-4o`, `gemini`, `claude`) |
