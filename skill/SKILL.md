...
description: |
  ...
  links as markdown files the user owns. You write event files directly and
  query them with your file tools. When the user states something worth
  remembering, capture it silently. A lost thought is worse than a stray event.
triggers:
  # Capture intent
  - remember that
  - remember this
...
| `source` | Yes | `agent` |
| `agent` | Yes | Your name (e.g. `cowork`, `claude-code`, `manus`) |
